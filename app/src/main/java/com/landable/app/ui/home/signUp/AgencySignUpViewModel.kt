...
class AgencySignUpViewModel : ViewModel() {
    ...
    fun onSignUpButtonClick(view: View) {
        when {
            agencyName.isNullOrEmpty() -> {
                listener?.onFailure("Please enter agency name.", "signUpButtonClick")
                return
            }

            agencyName?.length!! > 40 -> {
                listener?.onFailure(
                    "Please enter valid agency name. It should not more than 25 character.",
                    "signUpButtonClick"
                )
                return
            }

            personName.isNullOrEmpty() -> {
                listener?.onFailure("Please enter person name.", "signUpButtonClick")
                return
            }

            personName?.length!! > 40 -> {
                listener?.onFailure(
                    "Please enter valid person name. It should not more than 25 character.",
                    "signUpButtonClick"
                )
                return
            }
        ...
        try {
            val registerUserResponse = RegisterRepository().agencyUserRegister(
