...
class ProjectDetailDeepLinkFragment : Fragment(), ProjectDetailListener, AdvertisementClickListener,
    ...
    companion object {
        fun newInstance() = ProjectDetailDeepLinkFragment()
    }
    ...
    override fun onCreateView(
        ...
        binding.viewOnMap.setOnClickListener {
            val url = "http://maps.google.com/maps?daddr=${projectData!!.details.Fulladdress}"
        ...
        binding.llOwnerPRofile.setOnClickListener {
            if (AppInfo.getSCode() == "" || AppInfo.getSCode() == "0") {
                ...
                loadAgentProfileFragment(projectData!!.details.addedbyid)
            }
        }

        binding.ivShare.setOnClickListener {
            val intent = Intent(Intent.ACTION_SEND)
            val link = projectData!!.details.linkurl
            val name = projectData!!.details.projectname
        ...
        binding.llContactOwner.setOnClickListener {
            if (AppInfo.getSCode() == "" || AppInfo.getUserId() == "0") {
                ...
                val dialogFragment = ContactOwnerDialogFragment(
                    agentNumber,
                    projectData!!.details.name, agentMail, projectData!!.details.addedbyid
                )
                dialogFragment.show(fm, "")
            }
        }
            ...
            if (AppInfo.getSCode() == "" || AppInfo.getUserId() == "0") {
                ...
                addToFavourite(
                    PropertyDetailFragment.AddtoFavouriteDataModel(
                        projectData!!.details.id.toString(),
                        "property"
                    )
                )
            }
    ...
    private fun contactOwner() {
        (activity as HomeActivity).propertyID = projectid
        (activity as HomeActivity).contactType = "Project"
        (activity as HomeActivity).agentID = projectData!!.details.addedbyid
    }
    ...
    private fun updateProjectDetailUI() {
        ...
        binding.tvPropertyHeading.text = projectData!!.details.projectname
        binding.ivPropertyImage.load(LandableConstants.Image_URL + projectData!!.details.image1)
/*
*/


        if (projectData!!.details.isfavourite == "true") {
        ...
        binding.customerType.text = projectData!!.details.customertype
        binding.tvPrice.text =
            "\u20B9 " + projectData!!.details.mincost + " - " + projectData!!.details.maxcost
        //  binding.tvBathroomCount.text = projectDetailInfoModel!!.bathroom
        //   binding.tvBedroomCount.text = projectDetailInfoModel!!.bedroom
        binding.tvCategoryName.text = projectData!!.details.categoryname
        binding.tvPropertyID.text = projectData!!.details.projectid
        binding.tvPropertyPrice.text =
            "\u20B9 " + projectData!!.details.mincost + " - " + projectData!!.details.maxcost
        binding.tvPropertyLocation.text = projectData!!.details.Fulladdress
        binding.tvPossession.text = projectData!!.details.possessionname
        binding.tvPropertyType.text = projectData!!.details.subcategoryname
        binding.tvPossession.text = projectData!!.details.possessionname
        binding.tvOwnerName.text = projectData!!.details.name
        binding.tvLocation.text = projectData!!.details.Fulladdress
        binding.tvRating.text = projectData!!.details.rating.toString()
        // binding.propertyStrength.text = projectDetailInfoModel!!.strengthmsg
        binding.tvDiscription.text = projectData!!.details.description
        ...
        propertyResponse.observe(viewLifecycleOwner) {
            ...
            if (it == LandableConstants.noInternetErrorMessage) {
                ...
                try {
                    ...
                    addedbyid = projectData!!.details.addedbyid

                    if (projectData!!.details.lat.isNullOrEmpty() || projectData!!.details.lon.isNullOrEmpty()) {
                        lat = 28.7041
                        lon = 77.1025
                    } else {
                        lat = projectData!!.details.lat.toDouble()
                        lon = projectData!!.details.lon.toDouble()
                    }

                    (activity as HomeActivity).enableBackButton(projectData!!.details.projectname)
    ...
    override fun onLocationAdded(title: String, address: String, latlong: LatLng) {
        var distance = SphericalUtil.computeDistanceBetween(
            latlong,
            getLocationFromAddress(projectData!!.details.Fulladdress)
        )
        distance /= 1000.0

        postAddLocation(
            PropertyDetailFragment.PostAddLocation(
                projectData!!.details.projectid, "",
